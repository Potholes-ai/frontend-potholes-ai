...
socket.on("message", (stringObj) => {
  ...
  if (obj.messageType !== "simpleText") {
    const pos = JSON.parse(obj.message);
    console.log(pos);
    new L.marker([pos.latitude, pos.longitude], {
      icon: orangeIcon,
    }).addTo(map);
    map.setView([pos.latitude, pos.longitude], 3);
  } else {
  ...
  fetch("http://localhost:3000/", { method: "GET" })
    ...
    .then((data) => {
      ...
      data.forEach((element) => {
        const { latitude, longitude, image } = element;
        if (image === "") {
